...
Grid v2.0 - 32 coded variables + 6 rationale fields, across OS1 / OS2 / OS3.
...
C3 — Intervention specific example / short label:
  A short label (2-6 words, noun phrase — NOT a full sentence) for the specific intervention
  described in the article, matched to the C1 category assigned.
  -> Default case: reuse one of the "e.g." examples listed above for that category, as-is, no tag needed.
  -> Exception: if the specific intervention is not among those examples but clearly fits the
    category's logic, coin a short analogous label (e.g. rainwater harvesting basin ; green belt ;
    naturalised cemetery path) AND append " [analogous]" at the end of the string.
  -> Never write a narrative paragraph in C3.
...
def _build_system_prompt() -> str:
    # Format the grid as a readable CSV block for the model
    header = "Code,Section,Variable,Definition,Options"
    rows = "\n".join(
        f"{r['Code']},{r['Section']},{r['Variable']},{r['Definition']},{r['Options']}"
        for r in GRID
    )
    grid_block = f"{header}\n{rows}"
...
def single_shot(example: dict) -> list[dict]:
    ...
    """
    answer = {r["Code"]: example.get(r["Code"], "not reported") for r in GRID}
    return [
