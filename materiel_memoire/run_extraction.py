...
from prompt_extraction_v3 import (
    generate_messages,
    build_passages,
    grid_key,
    GRID,
...
def index_article(article_id: str, full_text: str, chunk_size=500, overlap=50):
    ...
    print(f"Indexé : {len(chunks)} chunks")


def get_header_chunks(article_id: str, n=4) -> list[str]:
    """Return chunks c0...c(n-1) in document order."""
    requested_ids = [f"{article_id}_c{i}" for i in range(n)]
    result = collection.get(ids=requested_ids, include=["documents"])
    by_id = dict(zip(result.get("ids", []), result.get("documents", [])))
...
def fallback_field(field_key: str, article_id: str, full_text: str = None) -> str:
    """
    ...
    if full_text:
        context = " ".join(full_text.split()[:2000])
    else:
        context = "\n\n---\n\n".join(get_header_chunks(article_id, n=4))
...
def extract_article(article_id: str, few_shot_examples: list = None, raw_text: str = None,
    ...
    if full_text and len(full_text.split()) <= MAX_WORDS_FULL_TEXT:
        ...
        header_chunks = get_header_chunks(article_id, n=4)
    ...
    expected_keys = {grid_key(row) for row in GRID}
    missing_keys = expected_keys - set(result)
    unexpected_keys = set(result) - expected_keys
    if missing_keys or unexpected_keys:
        raise ValueError(
            "Invalid extraction keys. "
            f"Missing: {sorted(missing_keys)}; unexpected: {sorted(unexpected_keys)}"
        )

    # Harmonise generic missing-value labels before writing the final CSV.
    for key, value in result.items():
        if isinstance(value, str) and value.strip().lower() in {
            "not-reported", "not described", "not found", "none", "null"
        }:
            result[key] = "not reported"
    ...
    if _doi_prefill:
        ...
        if current_a1 in ("not reported", "not found", "", "null", "none") or            not current_a1.startswith("http"):
            ...
            print(f"DOI injecté : {_doi_prefill}")
    ...
    for field_key, _ in CRITICAL_FIELDS.items():
        ...
        if current_val in ("not reported", "not found", "", "null", "none"):
            print(f"Fallback pour {field_key}...")
            recovered = fallback_field(field_key, article_id, full_text=full_text)
            if recovered.strip().lower() in {
                "not-reported", "not described", "not found", "none", "null"
            }:
                recovered = "not reported"
            print(f"Valeur récupérée : {recovered}")
            result[field_key] = recovered

    return result

# Convertir le résultat JSON → ligne template
...
def write_template_csv(rows: list[dict], output_path: str):
    ...
    print(f"CSV écrit : {output_path} ({len(rows)} articles)")
...
def _load_done_ids(output_path: str) -> set[str]:
    ...
    with open(path, newline="", encoding="utf-8-sig") as f:
        ...
        # Fallback : pas de colonne _article_id (ancien CSV)
            ...
            # On ne peut pas skip de façon fiable → on retourne vide
            pass
...
def run_pipeline(full_texts_dir: str, output_path: str, few_shot_examples=None):
    ...
    if done_ids:
        print(f"{len(done_ids)} articles déjà extraits — ils seront ignorés")

    files = sorted(
        list(full_texts_dir.glob("*.pdf")) +
        list(full_texts_dir.glob("*.txt"))
    )
    print(f"{len(files)} fichiers trouvés dans {full_texts_dir}")
    ...
    for f in files:
        ...
        if f.suffix == ".pdf":
            ...
            print(f"DOI trouvé dans le texte (p.1-2) : {doi_own_text or '(aucun)'}")
            if not doi_own_text:
                print(f"DOI via lien PDF (p.1-2)        : {doi_from_links or '(aucun)'}")
        ...
        if len(full_text_clean.split()) < 200:
            print(f"Texte trop court ({len(full_text_clean.split())} mots) — ignoré")
            continue

        # Indexer uniquement les articles qui dépassent le seuil du texte intégral.
        if len(full_text_clean.split()) > MAX_WORDS_FULL_TEXT:
        ...
        try:
            ...
            n_extracted += 1
            print(f"Extrait et sauvegardé ({n_extracted} total)")
    ...
    print(f"\nPipeline terminé : {n_extracted} nouveaux + {n_skipped} déjà faits")
...
if __name__ == "__main__":
    ...
    expected_example_keys = {grid_key(row) for row in GRID} | {"passages"}
    for index, example in enumerate(few_shot_examples, start=1):
        missing = expected_example_keys - set(example)
        extra = set(example) - expected_example_keys
        if missing or extra:
            raise ValueError(
                f"Invalid few-shot example {index}. "
